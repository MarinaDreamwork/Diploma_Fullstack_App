...
const Card = () => {
  ...
  return (
    item &&
    item.map(i => <div key={i.id} className='container card'>
      <div className='card-path w-100 m-2 p-2'>
        <NavLink to='/genres' className='path-text'>{i.category}</NavLink> {'>'} <a>{i.subCategory}</a>{'>'} {i.book_title}
      </div>
          ...
          <div className='d-flex align-self-center'>
            {(!inCart)
              ...
              <>
                ...
                <Favorite
                  style={{ fontSize: '2rem', color: 'red', paddingTop: '20px', paddingLeft: '15px' }}
